...
enum NucleiScanPlotRenderer {
    static func render(records: [NucleiScanRecord], selectedCombo: Int? = nil) -> NSImage? {
        guard !records.isEmpty else { return nil }
        let sorted = records.sorted { $0.comboIndex < $1.comboIndex }
        let width = 1120
        let height = 560
        let marginLeft: CGFloat = 108
        ...
        let minCount = sorted.map(\.count).min() ?? 0
        let best = sorted.max {
        ...
        for (idx, record) in sorted.enumerated() {
            ...
            let h = CGFloat(record.count) / CGFloat(maxCount) * plotHeight
            let rect = NSRect(x: x - barWidth / 2, y: marginBottom, width: barWidth, height: h)
            let color = colorForCount(record.count, minCount: minCount, maxCount: maxCount)
        ...
        let title = "Nuclei parameter scan"
        title.draw(
        ...
        "Nuclei count".draw(
            in: NSRect(x: 4, y: marginBottom + plotHeight / 2 - 40, width: 106, height: 44),
            withAttributes: [.font: NSFont.systemFont(ofSize: 22), .foregroundColor: NSColor.secondaryLabelColor, .paragraphStyle: paragraph]
        )

        if let best {
            let text = "Recommended: combo \(best.comboIndex), \(best.count) nuclei"
            text.draw(
                in: NSRect(x: marginLeft, y: CGFloat(height) - marginTop - 28, width: plotWidth, height: 28),
                withAttributes: [.font: NSFont.systemFont(ofSize: 20, weight: .medium), .foregroundColor: NSColor.systemRed]
            )
        }
    ...
    private static func colorForCount(_ count: Int, minCount: Int, maxCount: Int) -> NSColor {
        let denominator = max(1, maxCount - minCount)
        let normalized = CGFloat(count - minCount) / CGFloat(denominator)
        let hue = 0.62 - (0.62 * normalized)
